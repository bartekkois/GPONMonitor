...
{
    public class H640GOnuFactory : OnuFactory
    {
        public H640GOnuFactory(IResponseDescriptionDictionaries responseDescriptionDictionaries, IMapper mapper, IDataService snmpDataService) : base(responseDescriptionDictionaries, mapper, snmpDataService) { }

        public override IOnuFactory BuildOnu(uint oltId, uint oltPortId, uint onuId)
        {
            H640GOnu onu = _mapper.Map<H640GOnu>(base.BuildOnu(oltId, oltPortId, onuId));
