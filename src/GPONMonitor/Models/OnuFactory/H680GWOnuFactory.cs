...
{
    public class H680GWOnuFactory : OnuFactory
    {
        public H680GWOnuFactory(IResponseDescriptionDictionaries responseDescriptionDictionaries, IMapper mapper, IDataService snmpDataService) : base(responseDescriptionDictionaries, mapper, snmpDataService) { }

        public override IOnuFactory BuildOnu(uint oltId, uint oltPortId, uint onuId)
        {
            H680GWOnu onu = _mapper.Map<H680GWOnu>(base.BuildOnu(oltId, oltPortId, onuId));
