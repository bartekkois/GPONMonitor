...
{
    public class HL4GQVOnuFactory : OnuFactory
    {
        public HL4GQVOnuFactory(IResponseDescriptionDictionaries responseDescriptionDictionaries, IMapper mapper, IDataService snmpDataService) : base(responseDescriptionDictionaries, mapper, snmpDataService) { }

        public override IOnuFactory BuildOnu(uint oltId, uint oltPortId, uint onuId)
        {
            HL4GQVOnu onu = _mapper.Map<HL4GQVOnu>(base.BuildOnu(oltId, oltPortId, onuId));
