...
{
    public class LXT010GDOnuFactory : OnuFactory
    {
        public LXT010GDOnuFactory(IResponseDescriptionDictionaries responseDescriptionDictionaries, IMapper mapper, IDataService snmpDataService) : base(responseDescriptionDictionaries, mapper, snmpDataService) { }

        public override IOnuFactory BuildOnu(uint oltId, uint oltPortId, uint onuId)
        {
            LXT010GDOnu onu = _mapper.Map<LXT010GDOnu>(base.BuildOnu(oltId, oltPortId, onuId));
