...
{
    public class H665OnuFactory : OnuFactory
    {
        public H665OnuFactory(IResponseDescriptionDictionaries responseDescriptionDictionaries, IMapper mapper, IDataService snmpDataService) : base(responseDescriptionDictionaries, mapper, snmpDataService) { }

        public override IOnuFactory BuildOnu(uint oltId, uint oltPortId, uint onuId)
        {
            H665Onu onu = _mapper.Map<H665Onu>(base.BuildOnu(oltId, oltPortId, onuId));
