...
{
    public class LXT010SHOnuFactory : OnuFactory
    {
        public LXT010SHOnuFactory(IResponseDescriptionDictionaries responseDescriptionDictionaries, IMapper mapper, IDataService snmpDataService) : base(responseDescriptionDictionaries, mapper, snmpDataService) { }

        public override IOnuFactory BuildOnu(uint oltId, uint oltPortId, uint onuId)
        {
            LXT010SHOnu onu = _mapper.Map<LXT010SHOnu>(base.BuildOnu(oltId, oltPortId, onuId));
