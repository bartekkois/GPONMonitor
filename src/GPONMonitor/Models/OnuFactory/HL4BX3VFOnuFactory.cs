...
{
    public class HL4BX3VFOnuFactory : OnuFactory
    {
        public HL4BX3VFOnuFactory(IResponseDescriptionDictionaries responseDescriptionDictionaries, IMapper mapper, IDataService snmpDataService) : base(responseDescriptionDictionaries, mapper, snmpDataService) { }

        public override IOnuFactory BuildOnu(uint oltId, uint oltPortId, uint onuId)
        {
            HL4BX3VFOnu onu = _mapper.Map<HL4BX3VFOnu>(base.BuildOnu(oltId, oltPortId, onuId));
