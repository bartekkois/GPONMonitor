...
{
    public class HL4GXVFOnuFactory : OnuFactory
    {
        public HL4GXVFOnuFactory(IResponseDescriptionDictionaries responseDescriptionDictionaries, IMapper mapper, IDataService snmpDataService) : base(responseDescriptionDictionaries, mapper, snmpDataService) { }

        public override IOnuFactory BuildOnu(uint oltId, uint oltPortId, uint onuId)
        {
            HL4GXVOnu onu = _mapper.Map<HL4GXVOnu>(base.BuildOnu(oltId, oltPortId, onuId));
