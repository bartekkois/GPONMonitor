...
{
    public class HL4GMVROnuFactory : OnuFactory
    {
        public HL4GMVROnuFactory(IResponseDescriptionDictionaries responseDescriptionDictionaries, IMapper mapper, IDataService snmpDataService) : base(responseDescriptionDictionaries, mapper, snmpDataService) { }

        public override IOnuFactory BuildOnu(uint oltId, uint oltPortId, uint onuId)
        {
            HL4GMVROnu onu = _mapper.Map<HL4GMVROnu>(base.BuildOnu(oltId, oltPortId, onuId));
