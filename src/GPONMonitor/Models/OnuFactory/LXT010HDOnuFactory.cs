...
{
    public class LXT010HDOnuFactory : OnuFactory
    {
        public LXT010HDOnuFactory(IResponseDescriptionDictionaries responseDescriptionDictionaries, IMapper mapper, IDataService snmpDataService) : base(responseDescriptionDictionaries, mapper, snmpDataService) { }

        public override IOnuFactory BuildOnu(uint oltId, uint oltPortId, uint onuId)
        {
            LXT010HDOnu onu = _mapper.Map<LXT010HDOnu>(base.BuildOnu(oltId, oltPortId, onuId));
