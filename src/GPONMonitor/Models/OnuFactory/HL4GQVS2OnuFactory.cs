...
{
    public class HL4GQVS2OnuFactory : OnuFactory
    {
        public HL4GQVS2OnuFactory(IResponseDescriptionDictionaries responseDescriptionDictionaries, IMapper mapper, IDataService snmpDataService) : base(responseDescriptionDictionaries, mapper, snmpDataService) { }

        public override IOnuFactory BuildOnu(uint oltId, uint oltPortId, uint onuId)
        {
            HL4GQVSOnu onu = _mapper.Map<HL4GQVSOnu>(base.BuildOnu(oltId, oltPortId, onuId));
